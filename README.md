...
这是一个基于 Nuxt 3 的高度工程化的 Monorepo 项目，使用 pnpm workspaces 和 Turborepo 进行管理。项目采用 **Nuxt Layers** 架构，实现了代码的高度复用和应用间的解耦。
...
### 1. Nuxt Layers 继承机制
本项目利用 Nuxt 3 的 `extends` 特性。`packages/shared` 作为一个基础层，包含了所有应用通用的配置、模块和 UI 逻辑。
